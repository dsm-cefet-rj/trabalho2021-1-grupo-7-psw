...
router.get("/vendas/evento/:id", (req, res) => {
  let { id } = req.params;
  id = parseInt(id);
  ...
  const usersEvent = UserCompraEvento.filter((value) => value.idEvent === id);

  if (usersEvent.length === 0) {
    res.status(404).json({ msg: "Nao há vendas relacionadas a esse evento" });
    return;
  }
  res.json(usersEvent);
});

...
class Event{
    ...
    async update(name, slug, type, num_tickets,date, price, description){
        let event = this.events.find(event=> event.slug == slug)

        try{
            event.name = name
            event.slug = Slugify(name).toLowerCase()
            event.type = type
    ...
    async delete(slug){
        let index = this.events.findIndex(event=>event.slug === slug)
